...
export const validateOnUserRegistration = (name: string,
   value: any,
   errors: any) => {
   switch (name) {
      case "firstName":
      ...
      case "password":
         if (value.trim() === null || value.trim() === "" || value === undefined) {
            ...
            errors.password = "Password length must be atleast 6 letters";
         } else if (value.length > 16) {
            errors.password = "Password length must be less than or or equal to 16";
         } else {
